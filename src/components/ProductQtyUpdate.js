...
import { updateCartItems, productQtyDetails, addSimpleProductToCart, getCartDetails } from "../actions/AddcartAction";
...
function ProductDetails(props) {  
  ...
  useEffect(() => {             
    dispatch(productDetails(props.match.params.product_sku));   
    dispatch(getCartDetails(selector.addcart.cartId));     
  },[]);

//Get selected cart_items quantity
  useEffect(() => {                 
      const data = selector.addcart.items.map(item => {        
          if(item.id == props.match.params.cart_item_Id){
...
const updateCart = (qty) => {
    dispatch(updateCartItems(selector.addcart.cartId, props.match.params.cart_item_Id, qty));
  }
